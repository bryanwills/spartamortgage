...
const AIChatbot: React.FC<ChatbotProps> = ({ onContactRequest }) => {
  ...
  const websiteContent = {
    mortgageCalculator: {
      ...
      description: "Interactive calculator to estimate monthly payments, total interest, and see how extra payments can save thousands.",
      features: ["Loan amount calculation", "Interest rate analysis", "Extra payment benefits", "Amortization charts", "Payment comparison"]
    },
    loanPrograms: {
      title: "Loan Programs",
      description: "Comprehensive mortgage solutions including conventional, FHA, VA, USDA, and jumbo loans.",
      programs: ["Conventional Loans (3-20% down)", "FHA Loans (3.5% minimum down)", "VA Loans (0% down for veterans)", "USDA Loans (0% down for rural areas)", "Jumbo Loans (high-value properties)", "Refinance options"]
    },
    application: {
      title: "Mortgage Application",
      description: "Streamlined application process with access to multiple loan programs and competitive rates.",
      process: ["Online application", "Document upload", "Credit check", "Rate lock", "Closing coordination"]
    },
    company: {
      name: "Sparta Mortgage",
      description: "Professional mortgage services with competitive rates and personalized solutions.",
  ...
  const getContextualResponse = (userMessage: string): string => {
    ...
    if (lowerMessage.includes('calculator') || lowerMessage.includes('calculate') || lowerMessage.includes('payment')) {
    ...
    // Loan programs
    if (lowerMessage.includes('loan') || lowerMessage.includes('program') || lowerMessage.includes('fha') || lowerMessage.includes('va') || lowerMessage.includes('conventional')) {
    ...
    // Company information
    if (lowerMessage.includes('company') || lowerMessage.includes('sparta') || lowerMessage.includes('about')) {
        ...
        <div className="fixed bottom-24 right-6 z-40 w-96 h-[500px] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col">
          ...
          <div className="bg-gradient-to-r from-red-600 to-yellow-500 text-white p-4 rounded-t-2xl">
            ...
            <p className="text-sm text-white/90 mt-1">
              Ask me about our mortgage services, loan programs, or application process
            </p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.length === 0 && (
              <div className="text-center text-gray-500 dark:text-gray-400 py-8">
                ...
                <p className="text-sm">Hello! I'm here to help with your mortgage questions.</p>
                <p className="text-xs mt-1">Ask about our loan programs, calculator, or application process.</p>
              </div>
            )}
